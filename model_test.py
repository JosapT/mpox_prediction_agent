...
def add_engineered_features(df, target_column):
    ...
    if "systemic_illness_none_reported_or_missing" in df.columns:
        df["has_reported_systemic_illness"] = 1 - df["systemic_illness_none_reported_or_missing"]
    else:
...
def main():
    ...
    print("==============================")
    print(results_df.sort_values(by="balanced_accuracy", ascending=False).to_string(index=False))

    print("\n==============================")
    print("MODEL COMPARISON BY MACRO F1")
    print("==============================")
    print(results_df.sort_values(by="macro_f1", ascending=False).to_string(index=False))
